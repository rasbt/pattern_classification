...
the evidence can be calculated as follows ($\omega_j^C$ stands for "complement" and basically translate to "\textbf{not} class $\omega_j$." ):

\begin{equation}P(\textbf x_i) = P(\textbf x_i \mid \omega_j) \cdot P(\omega_j) \cdot P(\textbf x_i \mid \omega_j^C) \cdot P(\omega_j^C)\end{equation}
...
\subsubsection{Additive Smoothening}
\label{sec:additive_smoothening}
...
In order to avoid the problem of \emph{zero} probabilities, an additional smoothening term can be added to the \emph{multinomial Bayes} model. The most common variants of additive smoothening are the so-called \emph{Lidstone smoothening} ($\alpha<1$) and  \emph{Laplace smoothening} ($\alpha=1$).
...
\item $\alpha$: Parameter for additive smoothening. 
...
\subsection{Mutli-variate Bernoulli Naive Bayes}
\label{sec:bernoulli_bayes}

The \emph{Multi-variate Bernoulli} model is based on binary data: Every token in the feature vector of a document is associated with the value 1 or 0. The  feature vector has $d$ dimensions where $d$ is the number of words in the whole vocabulary (in Section \ref{sec:the_bag_of_words_model}); the value 1 means that the word occurs in the particular document, and 0 means that the word does not occur in this document. Empirical studies showed that the mutli-variate Bernoulli model is inferior to the multinomial Bayes model, and the latter has been shown to reduce the error by 27\% on average \cite{mccallum1998comparison}. The Bernoulli trials can be written as
...
\item  +1 and +2 are the parameters of \emph{Laplace smoothening} (Section \ref{sec:additive_smoothening}).
\end{itemize}
...
\item $\alpha$: An addititive smoothening parameter ($\alpha = 1$ for Laplace smoothening).
